...
test('個人購票待辦不會被購物詞誤擋',()=>assert.equal(isDefiniteJunkNotification(item('明天 20:00 前記得購買回程車票')),false));
test('已存在的帳單即使含優惠資訊仍保留',()=>assert.equal(isDefiniteJunkNotification(item('您的帳單已產生，9/20 前付款可享回饋')),false));
