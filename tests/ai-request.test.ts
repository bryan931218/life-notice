...
test('不接受 null 工具參數',()=>assert.throws(()=>parseAiToolCall({name:'create_calendar_event',arguments:'null'}),/格式/));
test('不將二月三十日自動改成三月',()=>assert.throws(()=>parseAiToolCall({name:'create_calendar_event',arguments:JSON.stringify({start_at:'2026-02-30T09:00:00+08:00'})}),/日期/));
