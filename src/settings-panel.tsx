...
export function SettingsPanel({data,onChange,onRestore,onPrivacy,onErase}:{data:State;onChange:()=>void;onRestore:()=>void;onPrivacy:()=>void;onErase:()=>void}){
 ...
 return <>
  ...
  <View style={styles.summary}><View style={[styles.dot,{backgroundColor:access&&count>0?p.green:'#A47A34'}]}/><View style={s.flex}><Text style={s.cardTitle}>{Platform.OS!=='android'?'使用截圖或文字匯入':!access?'尚未允許讀取其他 App 通知':count===0?'通知存取已開，但尚未指定來源':`正在監聽 ${count} 個 App`}</Text><Text style={s.caption}>{Platform.OS==='android'&&count===0?'請在「管理監聽的 App」勾選 LINE、Messenger 等來源並儲存。':ai.enabled?'AI 語意辨識已啟用':'使用本機辨識'}</Text></View></View>
  ...
  <Group icon="sparkles-outline" color="#EBE5F5" title="AI 與行事曆" subtitle="辨識模式、API key、手機行事曆同步">
   <View style={styles.row}><View style={s.flex}><Text style={s.body}>AI 語意辨識</Text><Text style={s.caption}>開啟後採高召回模式：已選 App 中除明確垃圾訊息外都交給 AI 判斷。API 費用由你的帳戶支付。</Text></View><Switch accessibilityLabel="AI 語意辨識" disabled={busy} value={ai.enabled} onValueChange={enabled=>void perform(async()=>{if(enabled&&!hasKey)throw new Error('請先在下方驗證並儲存 API key。');if(enabled&&!await confirm('啟用後，已選 App 中除驗證碼、廣告與系統狀態等明確垃圾訊息外，通知內容與必要的近期事件摘要都會傳送至 OpenAI 分析，並產生 API 費用。'))return;const next={...ai,enabled};await api.setAiSettings(next);setAi(next);return enabled?'AI 高召回辨識已啟用。':'已切換成本機辨識。';})}/></View>
   ...
   {Platform.OS==='android'&&<View style={styles.row}><View style={s.flex}><Text style={s.body}>自動加入手機行事曆</Text><Text style={s.caption}>{auto?'已開啟：即使 App 沒有在畫面上，時間明確的新通知也會直接寫入。':'目前關閉：抓到通知後只會保存在 App，或由你點快捷按鈕加入。'}</Text></View><Switch accessibilityLabel="自動加入手機行事曆" disabled={busy} value={auto} onValueChange={enabled=>void perform(async()=>{const saved=await api.setAutoCalendarEnabled(enabled);setAuto(saved);if(enabled&&!saved)throw new Error('未取得行事曆權限，仍可保存在 App。');return saved?'已開啟背景自動加入行事曆。':'已關閉自動加入行事曆。';})}/></View>}
  </Group>
