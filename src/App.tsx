...
function Main(){
  ...
  const save=()=>void run(async()=>{
    ...
    if(draft.editing){
      ...
      next={...live.current,notices:live.current.notices.map(n=>n.id===old.id?updateNotice(n,{title,source:draft.source.trim(),sourceImage,category:draft.category,dueAt,needsReview:false,assignee:'我',checklist:checks,remindMinutes}):n)};
    }else{
      const item:Notice={id:id(),title,source:draft.source.trim(),sourceImage,category:draft.category,dueAt,needsReview:false,assignee:'我',checklist:checks,done:false,remindMinutes,createdAt:now,updatedAt:now,history:[]};
  ...
  const toggleAi=()=>void run(async()=>{if(!aiConfig.enabled&&!await api.hasOpenAiKey()){setShowAiKey(true);return}const next={...aiConfig,enabled:!aiConfig.enabled};await api.setAiSettings(next);setAiConfig(await api.getAiSettings())});
  ...
  const setupQuickCapture=()=>void run(async()=>{let calendarOk=autoCalendar;if(!calendarOk){calendarOk=await api.setAutoCalendarEnabled(true);setAutoCalendar(calendarOk)}if(!calendarOk)throw new Error('快速擷取需要行事曆權限。');const requested=api.requestQuickCaptureTile();if(!requested)notify('請打開 Android「快速設定」的編輯畫面，加入「擷取行程」。')});
  ...
  const todos=sorted.filter(n=>!n.dueAt&&!n.needsReview).slice(0,10);
  const needsReview=sorted.filter(n=>!!n.needsReview).slice(0,4);
  ...
  const card=(n:Notice)=><Pressable key={n.id} accessibilityRole="button" onPress={()=>setSelected(n.id)} style={s.card}><View style={s.row}><View style={s.categoryDot}><Icon name={n.importance==='urgent'?'alert-circle-outline':isAuto(n)?'sparkles-outline':'calendar-outline'}/></View><View style={s.flex}><Text numberOfLines={2} style={s.cardTitle}>{n.title}</Text><Text style={s.caption}>{whenLabel(n)}{n.location?` · ${n.location}`:''}</Text></View><Icon name="chevron-forward" color={p.muted} size={18}/></View></Pressable>;
    ...
    <KeyboardAvoidingView style={s.flex} behavior={Platform.OS==='ios'?'padding':undefined}>
      <ScrollView ref={scroll} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
        ...
        {tab==='manual'&&<>
          ...
          <Field label="名稱" value={draft.title} max={100} onChange={title=>patchDraft({title})} placeholder="例如：填寫畢業表單"/>
          <View style={s.row}><View style={{flex:1.35}}><Field label="日期（選填）" value={draft.date} max={10} onChange={date=>patchDraft({date})} placeholder="YYYY-MM-DD"/></View><View style={s.flex}><Field label="時間（選填）" value={draft.time} max={5} onChange={time=>patchDraft({time})} placeholder="HH:mm"/></View></View>
          {!draftIsTodo&&<><Text style={s.label}>提醒</Text><Chips<number|null> values={[[null,'不提醒'],[0,'準時'],[60,'1 小時前'],[1440,'1 天前']]} value={draft.remind} onChange={remind=>patchDraft({remind})}/></>}
