...
export function NoticeHistory({notices,onOpen}:{notices:Notice[];onOpen:(id:string)=>void}){
 ...
 const items=historyNotices(notices.filter(n=>!!n.dueAt),filter,query);
 return <View>
  ...
  {items.slice(0,limit).map(n=><Pressable key={n.id} accessibilityRole="button" accessibilityLabel={`${n.title}，${n.done?'已完成':'過期未完成'}，查看詳情`} onPress={()=>onOpen(n.id)} style={s.card}>
   ...
   <Text style={s.caption}>{`${new Date(n.dueAt!).getFullYear()}年 ${displayDay(new Date(n.dueAt!))} ${n.allDay?'全天':displayTime(new Date(n.dueAt!))}`}</Text>
   <Text style={[s.caption,{color:n.done?p.green:p.orange}]}>{n.done?'已完成':'過期未完成'} · {n.category}</Text>
  </Pressable>)}
  {items.length===0&&<View style={s.empty}><Icon name="archive-outline"/><Text style={s.emptyTitle}>{query.trim()?'找不到符合的紀錄':'目前沒有紀錄'}</Text><Text style={s.caption}>已移出的行程與過去日期的行程會保留在這裡。</Text></View>}
