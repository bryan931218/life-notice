...
import {Chips,Icon,p} from './ui';
import {NoticeHistory} from './history-ui';
...
export function MonthCalendar({notices,onOpen,onAdd}:{notices:Notice[];onOpen:(id:string)=>void;onAdd:(date:string)=>void}){
  ...
  const dayItems=useMemo(()=>noticesForDay(notices,selected,true).sort((a,b)=>(a.dueAt?Date.parse(a.dueAt):0)-(b.dueAt?Date.parse(b.dueAt):0)),[notices,selected]);
  ...
  return <View style={c.page}>
    <Text style={{fontSize:29,fontWeight:'800',color:p.ink}}>你的安排</Text><Chips<'calendar'|'history'> values={[["calendar","月曆"],["history","歷史紀錄"]]} value={view} onChange={setView}/>
    {view==='history'?<NoticeHistory notices={notices} onOpen={onOpen}/>:<>
    <View style={c.calendarCard}>
      ...
      <View style={c.grid}>{cells.map(d=>{
        const key=dayKey(d),items=noticesForDay(notices,key,true),inMonth=d.getMonth()===month.getMonth(),today=key===dayKey(now),active=key===selected;
        return <Pressable key={key} accessibilityRole="button" accessibilityState={{selected:active}} accessibilityLabel={`${key}，${items.length} 個行程`} onPress={()=>{setSelected(key);if(!inMonth)setMonth(new Date(d.getFullYear(),d.getMonth(),1))}} style={[c.day,active&&c.dayActive]}>
          ...
          {items.length>0?<View style={c.dots}>{items.slice(0,3).map(n=><View key={n.id} style={[c.dot,{backgroundColor:n.done?'#A8B3AE':active?'#E5C6AD':catColor[n.category]}]}/>)}{items.length>3&&<Text style={[c.more,active&&{color:p.green}]}>+{items.length-3}</Text>}</View>:<View style={c.dotSpace}/>}
        </Pressable>
      })}</View>
    </View>
    ...
    {dayItems.length>0?<View style={c.timeline}>{dayItems.map((n,index)=>{
      ...
      return <Pressable key={n.id} accessibilityRole="button" onPress={()=>onOpen(n.id)} style={c.eventRow}>
        ...
        <View style={c.eventCard}>
          <View style={c.eventTop}><View style={[c.categoryPill,{backgroundColor:catSoft[n.category]}]}><View style={[c.categoryMiniDot,{backgroundColor:catColor[n.category]}]}/><Text style={[c.categoryText,{color:catColor[n.category]}]}>{n.category}</Text></View>{n.done?<View style={[c.review,{backgroundColor:p.mint}]}><Text style={[c.reviewText,{color:p.green}]}>已完成</Text></View>:n.needsReview&&<View style={c.review}><Text style={c.reviewText}>待確認</Text></View>}</View>
    ...
    })}</View>:<View style={c.empty}><View style={c.emptyIcon}><Icon name="calendar-clear-outline" size={24}/></View><Text style={c.emptyTitle}>這天沒有安排</Text><Text style={c.emptyText}>點右上方新增，安排這一天。</Text></View>}
    </>}
  </View>;
}
...
const c=StyleSheet.create({
  ...
  weekRow:{flexDirection:'row',paddingHorizontal:2,marginBottom:3},week:{width:'14.2857%',textAlign:'center',fontSize:11,fontWeight:'700',color:'#81908B',paddingVertical:4},weekend:{color:'#A06C6C'},grid:{flexDirection:'row',flexWrap:'wrap'},day:{width:'14.2857%',height:48,borderRadius:15,alignItems:'center',justifyContent:'center'},dayActive:{backgroundColor:p.green},numWrap:{width:30,height:30,borderRadius:15,alignItems:'center',justifyContent:'center'},todayRing:{borderWidth:1.5,borderColor:p.green},dayNum:{fontSize:13,fontWeight:'700',color:p.ink},dayNumActive:{color:'white',fontWeight:'900'},muted:{color:'#C1CAC7'},dots:{height:9,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:2,marginTop:1},dot:{width:4.5,height:4.5,borderRadius:3},dotSpace:{height:10},more:{fontSize:8.5,fontWeight:'800',color:p.muted,marginLeft:1},
