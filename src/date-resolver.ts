...
export function resolveDateText(text:string,base=new Date()){
 ...
 for(const m of text.matchAll(numeric)){
  ...
  if(m[0].includes('.')&&!m[1]&&/(?:上午|早上|中午|下午|晚上|晚間|凌晨)\s*$/.test(before))continue;
  const year=Number(m[1]||base.getFullYear()),month=Number(m[2]),day=Number(m[3]),d=new Date(year,month-1,day);if(d.getMonth()!==month-1||d.getDate()!==day){invalid=true;continue;}dates.add(key(d));
 }
 ...
 for(const m of text.matchAll(/(上午|早上|中午|下午|晚上|晚間|凌晨|今晚|明晚)?\s*(\d{1,2}|[零〇一二兩三四五六七八九十]{1,3})\s*(?:[:：.．點時](半|\d{1,2}|[零〇一二兩三四五六七八九十]{1,3})?(?:分)?)/g)){
