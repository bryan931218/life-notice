...
export function EventEditor({draft,patch,onImage,onPaste,onSave,busy,message,warnings}:{draft:Draft;patch:(value:Partial<Draft>)=>void;onImage:()=>void;onPaste:()=>void;onSave:()=>void;busy:boolean;message:string;warnings:string[]}){
 ...
 return <>
  <Text style={s.eyebrow}>{draft.editing?'調整行程':'建立新行程'}</Text>
  <Text style={s.heading}>{draft.editing?'編輯行程':'新增行程'}</Text>
  ...
  <View style={e.section}>
   <Field label="行程名稱" value={draft.title} onChange={title=>patch({title})} max={100} placeholder="例如：和安玲吃飯"/>
  ...
  <View style={e.section}>
   ...
   <Field label="地點（選填）" value={draft.location} max={200} onChange={location=>patch({location})} placeholder="店名、地址或會議室"/>
...
export function DetailHeading({notice,when}:{notice:Notice;when:string}){
 return <View style={e.detail}>
  <View style={s.row}><View style={e.badge}><Text style={e.badgeText}>{notice.category}</Text></View><Text style={s.caption}>{notice.done?'已結束':notice.needsReview?'時間待確認':'行程'}</Text></View>
